...
/* Functions for RX/TX of letter */
void UART_Putch(char ch) // print one character
{
	while(!(UCSR0A & 0x20)); // check if the buffer is empty
							 // unless the buffer is empty, wait

	UDR0 = ch; // otherwise, ch is moved to UDR0(Buffer 0)
}
